...
function generateAllStudentsGrades(numStudents, numGrades) {
  ...
  for (let i = 0; i < numStudents; i++) {
    let studentGrades = generateStudentGrades(numGrades);
...
function calculateClassAverageBySubject(allGrades, numGrades) {
  ...
  for (let i = 0; i < numGrades; i++) {
    let gradesForSubject = allGrades.map((studentGrades) => studentGrades[i]);
    let averageForSubject = calculateAverage(gradesForSubject);
...
/**
 * Fonction pour donner une appréciation basée sur la moyenne
 * @param {number} average - La moyenne à évaluer
 * @returns {string} - L'appréciation correspondant à la moyenne donnée
 */
...
function main() {
  ...
  const allStudentGradesText = allGrades.map((studentGrades, i) => {
    let gradesText = studentGrades.map((grade, j) => `${subjects[j]}: ${grade}`);
    return `Notes de l'élève ${i + 1} :\n${gradesText.join('\n')}`;
  });
  ...
  const allClassAveragesBySubjectText = classAveragesBySubject.map(
    (average, i) => `Moyenne de la classe pour la matière ${subjects[i]} : ${average}`
  );
  alert(allClassAveragesBySubjectText.join('\n'));

  // Afficher les moyennes des élèves avec appréciation
  const studentsData = [];

  for (let i = 0; i < numStudents; i++) {
    let appreciation = getAppreciation(averages[i]);
    studentsData.push({
      student: i + 1,
      average: averages[i],
  ...
  // Trier les élèves en fonction de leur moyenne
  studentsData.sort((a, b) => b.average - a.average);

  const allStudentAveragesText = studentsData.map((data) => {
    return `Élève ${data.student}, Moyenne : ${data.average.toFixed(2)}, Appréciation : ${data.appreciation}`;
  });
